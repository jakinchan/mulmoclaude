...
**ファイル名は `<id>.json` にする**。一覧に出る id は**ファイルの中身**の `id` だが、`manageRoles` の update / delete は**ファイル名**で role を探す (`server/utils/files/roles-io.ts`)。手で置いた / 手でリネームしたファイルで両者が食い違うと **一覧に出ている id では `Role '<id>' not found.` になる**（ファイル名側の id なら操作できる。ただし ① ファイル名が `isValidRoleId` の `[a-zA-Z0-9_-]+` を外れる場合（`my role.json` 等）は `Invalid role id` で弾かれ**どちらの名前でも届かない**、② 逆に中身の `id` がパターンを外れる場合（`RoleSchema.id` は素の `z.string()` なので手置きファイルでは起こる）は `<id>.json` への rename が唯一の handle を潰すので id 側を直す、③ 中身の `id` が built-in と一致していると built-in を shadow するため一覧側の id は「built-in は消せない」で弾かれる。warn message は ①②③ と両方不正のケースを区別し、**実際に有効な直し方だけ**を出す）。同じ `id` を持つファイルが 2 つあると、**両方が一覧に出た上で** `getRole` の解決先は `readdir` 順で先に見つかった方になる。どちらも role を落とす理由にはならないので**警告だけ出す**（食い違い: ファイル名と `id` の両方 / 重複: 解決先と無視された側のファイル名）(#2656)。`manageRoles` 経由で作れば `saveRole(role.id, role)` なので常に一致する。

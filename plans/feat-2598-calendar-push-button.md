...
### 事前の書き込み可否チェックは「拒否の証拠がある時」だけ

`accessRole` を報告するのは `calendarList` だけなので、まずリストを引く。`reader` の
カレンダーはここで理由付きで拒否できる（イベントごとに不透明な 403 を食らうより良い）。

ただし**リストに無いことを read-only とみなしてはいけない**。`calendarList` は
ユーザーが「追加した」カレンダーしか持たないので、書き込み権限付きで共有されているだけの
カレンダーは載っていない。載っていないものを拒否すると、そのカレンダーでは機能が完全に使えなくなる。

なので `accessRole` は `string | null` にして、null（=リストに無い）は**不明**として通す。
timeZone は `calendars.get` で id 直指定で取る。実際に権限が無ければ Google が 403 を返すので、
それを「このカレンダーのイベントを変更する権限が無い可能性」というメッセージで報告する
（`googleApiError` は 403 に「API が有効か？」というヒントを足すが、権限 403 では誤誘導になる）。

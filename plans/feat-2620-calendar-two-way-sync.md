...
2b. **保護の適用範囲はレコードとベースラインで異なる**（Codex レビュー #2666 で判明）。

   1つのカレンダーは複数コレクションを支えうるので、あるコレクションの衝突は他の
   コレクションについて何も語らない。グループ単位で1つの集合を共有すると、
   **`autoPush` を宣言すらしていないコレクション**が隣の衝突で凍る（push しないので
   衝突しえないのに）。しかも同期トークンは進むので Google は再送しない。

   - **レコードの書き込み**: コレクション単位（`unpushedFor`）
   - **ベースライン**: グループ全体の和集合（`allUnpushed`）

   ベースラインを和集合にするのは、`.push-state.json` が**カレンダー単位で1つ**しか
   ベースラインを持たないため。コレクション単位のベースラインが存在しない以上、
   誰かの衝突が未解決のまま進めると 3 のサイレント上書きが起きる。据え置きは
   「衝突を報告し続ける」だけなので、共有ストレージが強いる非対称の安全側。

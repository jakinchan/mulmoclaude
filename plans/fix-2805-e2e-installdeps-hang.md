...
2. **ステップ単位の `timeout-minutes`** — **`install-deps` の3分だけ**。作業は20数秒なので十分で、
   万一詰まっても15分の予算を丸ごと失わない。

   当初はコールドDL側にも10分の上限を付けたが、**これはリグレッションだった**(Codex が blocking で指摘)。
   ワークフロー自身のコメントが「遅い CDN の日はダウンロードが約13分」(#1725)と記録しており、
   10分で切ると**それまで通っていたコールド実行を確実に殺す**。

   対処として **ダウンロードと apt を分離**した:
   - `playwright install`(`--with-deps` を外す)= ダウンロードのみ。**上限なし**
     (ジョブの15分が既に上限で、既知の最悪値を超える上限は飾りにしかならない)
   - `playwright install-deps` = apt のみ。3分の上限

   OS ライブラリはシステムパスに入りキャッシュに乗らないので、`install-deps` は
   **キャッシュヒット/ミスの両方で必要**。分離した結果 `if:` も外れて整理された。
...
**この PR で実証できないこと**: キャッシュミス経路は普段走らないので実走確認できない。
